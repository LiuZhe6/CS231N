...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # 设置所有层的维度大小,按照网络从左至右的顺序，将维度加到一个list中
        layers_dims = [input_dim] + hidden_dims + [num_classes]
        # 对每一层初始化参数
        for i in range(self.num_layers):
            self.params['W' + str(i + 1)] = np.random.normal(0, weight_scale, (layers_dims[i], layers_dims[i + 1]))
            self.params['b' + str(i + 1)] = np.zeros((1, layers_dims[i + 1]))
            # 对normalization设置初试参数
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization is not None:
                if self.normalization == 'batchnorm':
                    scores, bn_cache = batchnorm_forward(scores, self.params['gamma' + str(i + 1)],
            ...
            caches.append(cache)
            pass
        scores, fc_cache = affine_forward(scores, self.params['W' + str(self.num_layers)],
